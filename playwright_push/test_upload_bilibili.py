...
# ========== 可写死的变量 ==========
# 要上传的视频绝对路径
VIDEO_PATH = "/Users/hf/Downloads/4444.mp4"
...
WAIT_BEFORE_CLICK_SUBMIT_SEC = 5
...
def _get_first_article_row_text(page):
    """稿件管理页上取「第一个作品」整行文案（含状态如「审核中」「未通过」等）。"""
    selectors = [
...
def wait_for_audit_result(page, title):
    """
    上传成功后轮询：已通过 tab 第一个作品=本视频→通过；
    未通过 tab 第一个作品=本视频 且 该条非「审核中」→未通过；审核中则继续等。
...
def main(video_path_arg=None, title_arg=None):
    ...
    if not os.path.isfile(video_path):
        print("视频文件不存在: {}".format(video_path))
        sys.exit(1)

    cookies_list = get_cookies_list()
    if not cookies_list:
        print("未配置 Cookie。请填写 COOKIES_LIST（数组）或 COOKIE_FILE / COOKIES_DICT。")
        print("Cookie 为数组对象时，过期或账号限制会自动换下一个重试。")
        sys.exit(1)

    title = (title_arg or VIDEO_TITLE or "").strip() or os.path.splitext(os.path.basename(video_path))[0]
    last_error = None
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if last_error:
                raise last_error
            print("所有账号均未成功（Cookie 过期或账号限制），请检查配置或稍后重试。")
            sys.exit(1)
        finally:
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    main(video_path_arg=a.video, title_arg=a.title)
